import { Delete, Download, Edit, Search, Upload, View,Eraser } from "lucide-react";
...
const deleteAll = async () => {
  ...
  try {
    ...
    if (res.data.success) {
      ...
      setMyPhotos([]); // instantly update UI
    } else {
      ...
      <div className="  flex flex-col  pl-72 p-10  ">
        <div className="flex gap-[670px]  ">
          ...
          <div className="flex items-center mt-1 gap-3 text-white ">
            ...
            <button onClick={()=>navigate("/upload")}   className="flex cursor-pointer items-center bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg text-sm font-semibold">
        ...
        <div className="flex gap-[800px] mt-10 ">
          <div className="inline mt-1  text-white">
            <input type="checkbox" name="select" checked={checked} onChange={()=>setChecked(prev=> !prev)} id="" />
            <label className="pl-2">Select All</label>
          </div>

          <div className="flex items-center mt-1 gap-3 text-white ">
            <button onClick={clearEverything} className="flex cursor-pointer items-center text-white border border-gray-500 hover:bg-blue-700 px-3 rounded-lg py-2 text-sm font-semibold ">
              <Eraser className=" mr-2 " size={16} />
              Clear Selected
            </button>

            <button onClick={deleteAll} className="flex cursor-pointer items-center text-white border  border-gray-500  hover:bg-red-700 px-4 py-2 rounded-lg text-sm font-semibold">
